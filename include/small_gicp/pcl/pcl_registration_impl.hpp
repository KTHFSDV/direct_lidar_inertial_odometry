...
template <typename PointSource, typename PointTarget>
void RegistrationPCL<PointSource, PointTarget>::registerInputSource(const PointCloudSourceConstPtr& cloud) {
  if (input_ == cloud) {
    return;
  }
  pcl::Registration<PointSource, PointTarget, Scalar>::setInputSource(cloud);
...
template <typename PointSource, typename PointTarget>
void RegistrationPCL<PointSource, PointTarget>::registerInputTarget(const PointCloudTargetConstPtr& cloud) {
  if (target_ == cloud) {
    return;
  }
  pcl::Registration<PointSource, PointTarget, Scalar>::setInputTarget(cloud);
...
template <typename PointSource, typename PointTarget>
void RegistrationPCL<PointSource, PointTarget>::setSourceCovariances(std::shared_ptr<std::vector<Eigen::Matrix4d>>& covs) {
  if (input_ == nullptr) {
    PCL_ERROR("[RegistrationPCL::setSourceCovariances] Source cloud is not set\n");
    return;
  }
...
template <typename PointSource, typename PointTarget>
void RegistrationPCL<PointSource, PointTarget>::setTargetCovariances(std::shared_ptr<std::vector<Eigen::Matrix4d>>& covs) {
  if (target_ == nullptr) {
...
template <typename PointSource, typename PointTarget>
std::shared_ptr<std::vector<Eigen::Matrix4d>> RegistrationPCL<PointSource, PointTarget>::getSourceCovariances() const {
...
template <typename PointSource, typename PointTarget>
std::shared_ptr<std::vector<Eigen::Matrix4d>> RegistrationPCL<PointSource, PointTarget>::getTargetCovariances() const {
...
template <typename PointSource, typename PointTarget>
void RegistrationPCL<PointSource, PointTarget>::calculateSourceCovariances() {
  if (input_ == nullptr) {
    PCL_ERROR("[RegistrationPCL::calculateSourceCovariances] Source cloud is not set\n");
    return;
  }

  auto aligned_covs = std::make_shared<std::vector<Eigen::Matrix4d>>();
  PointCloudProxy<PointSource> source_proxy(*input_, *aligned_covs);
  
  estimate_covariances_omp(source_proxy, *source_tree_, k_correspondences_, num_threads_);
  source_covs_ = std::make_shared<std::vector<Eigen::Matrix4d>>(source_proxy.covs);

  if (verbose_) {
    PCL_INFO("[RegistrationPCL::calculateSourceCovariances] Estimated %lu source covariances\n", source_covs_->size());
  }
}

template <typename PointSource, typename PointTarget>
void RegistrationPCL<PointSource, PointTarget>::calculateTargetCovariances() {
  if (target_ == nullptr) {
    PCL_ERROR("[RegistrationPCL::calculateTargetCovariances] Target cloud is not set\n");
    return;
  }

  auto aligned_covs = std::make_shared<std::vector<Eigen::Matrix4d>>();
  PointCloudProxy<PointTarget> target_proxy(*target_, *aligned_covs);
  
  estimate_covariances_omp(target_proxy, *target_tree_, k_correspondences_, num_threads_);
  target_covs_ = std::make_shared<std::vector<Eigen::Matrix4d>>(target_proxy.covs);

  if (verbose_) {
    PCL_INFO("[RegistrationPCL::calculateTargetCovariances] Estimated %lu target covariances\n", target_covs_->size());
  }
}
...
template <typename PointSource, typename PointTarget>
void RegistrationPCL<PointSource, PointTarget>::setInitialLambdaFactor(double init_lambda_factor) {
...
template <typename PointSource, typename PointTarget>
void RegistrationPCL<PointSource, PointTarget>::setMaxCorrespondenceDistance(double distance) {
  if (distance <= 0.0) {
    PCL_ERROR("[RegistrationPCL::setMaxCorrespondenceDistance] Invalid correspondence distance: %f\n", distance);
    distance = 1.0;
  }
...
void RegistrationPCL<PointSource, PointTarget>::computeTransformation(PointCloudSource& output, const Matrix4& guess) {
  ...
  registration.optimizer.init_lambda = lm_lambda_;
  registration.optimizer.lambda_factor = lm_init_lambda_factor_;
